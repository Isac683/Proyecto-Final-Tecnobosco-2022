...
function generar1() {
    ...
    if (correo == "" || name == "") {
        ...
        if (contraseña != recontraseña || contraseña < 8) {
            document.getElementById("respuesta1").value = "Error, las contraseñas no son iguales (Revisa que la contraseña tenga al menos 8 caracteres)";
        } else {
...
function guardar_tareas() {
    ...
    if (txt != null) {
        ...
        for (var i = 0; i < guardados.length; i++) {
            var nombre_profesor = guardados[i].name
        }
...
function crearTarjeta(name, correo, telefono, ident, columna) {
    var datos = document.createElement("div");
    datos.setAttribute("class", "tarjeta");
    datos.setAttribute("id", ident);
    ...
    parr.innerHTML = correo;
    var dat = document.createElement("div");
    var par = document.createElement("p");
    par.innerHTML = telefono;

    dat.appendChild(parr);
    dat.appendChild(par);
    datos.appendChild(dat);

    columna.appendChild(datos);
}

function registradas1() {
    var txt = localStorage.getItem("datos");
    //var creadas = document.getElementById("tar_crea");
    var donde = document.getElementById("tar_crea");
    ...
    if (txt != null) {
        ...
        for(var i=0;i<guardados.length;i++){
            var actual= guardados[i];
            crearTarjeta(actual.name, actual.correo, actual.telefono, actual.ident, donde);
